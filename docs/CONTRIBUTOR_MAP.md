...
|---|---|---|
| Economic constants & defaults | 1–50 | `MIN_CAP_VALUE`, `DEFAULT_MAX_PRECISION_PARTICIPANTS`, `CONFIG_TIMELOCK_LEDGERS`, etc. |
| Initialization & schema | 60–130 | `initialize`, `get_schema_version`, `migrate_schema_v1_to_v2` |
| Pause/unpause | 130–165 | `is_paused`, `pause_contract`, `unpause_contract` |
| Round management | 165–270 | `create_round`, `get_active_round`, `get_last_round_id` |
| Archived rounds | 270–350 | `get_archived_round`, `get_recent_archived_rounds` |
| Access control queries | 350–380 | `get_admin`, `get_oracle` |
| Oracle deviation guardrails | 380–440 | `set_oracle_max_deviation_bps`, `get_oracle_max_deviation_bps`, `arm_oracle_deviation_override` |
| Oracle heartbeat | 440–530 | `update_oracle_heartbeat`, `get_oracle_heartbeat`, `is_oracle_live`, `set_oracle_stale_threshold`, `get_oracle_stale_threshold` |
| Window management | 530–540 | `set_windows` (schedules via timelock) |
| Economic controls | 540–590 | `set_max_stake`, `get_max_stake`, `set_max_user_exposure`, `get_max_user_exposure`, `set_max_pending_winnings` |
| Timelocked config governance | 590–720 | `schedule_windows`, `schedule_max_stake`, `schedule_max_user_exposure`, `schedule_max_pending_winnings`, `schedule_oracle_stale_threshold`, `schedule_oracle_deviation_bps`, `get_pending_config_change`, `apply_scheduled_changes`, `cancel_config_change` |
| Minimum participants | 720–760 | `set_min_participants`, `get_min_participants` |
| Precision participant cap | 760–800 | `set_max_precision_participants`, `get_max_precision_participants` |
| User stats & winnings | 800–830 | `get_user_stats`, `get_pending_winnings` |
| Betting (Up/Down) | 830–920 | `place_bet` |
| Precision prediction | 920–1020 | `place_precision_prediction`, `predict_price` |
| Commit-reveal flow | 1020–1200 | `commit_prediction`, `reveal_prediction` |
| Position queries | 1200–1270 | `get_user_position`, `get_user_precision_prediction`, `get_precision_predictions` |
| Paginated queries | 1270–1400 | `get_updown_positions_paginated`, `get_precision_predictions_paginated`, `get_round_participants_paginated` |
| Resolution & settlement | 1450–1800 | `resolve_round` (UpDown + Precision payout logic), `cancel_round`, `_accumulate_pending` |
| Balance & minting | 1800–1900 | `balance`, `mint_initial`, `_set_balance`, `_add_balance` |
| Claim winnings | 1900–1950 | `claim_winnings` |
| Private helpers | 1950–2100 | `assert_no_active_round`, `_ensure_not_paused`, `_require_supported_schema`, `_schema_version`, `_extend_persistent_ttl`, `_validate_windows`, `_validate_max_stake`, `_validate_oracle_stale_threshold`, `_validate_oracle_max_deviation_bps`, `_schedule_config_change`, `_apply_config_payload`, `_validate_oracle_payload_context`, `_update_stats_win`, `_update_stats_loss`, `_archive_round`, `payout_add`, `payout_mul` |
